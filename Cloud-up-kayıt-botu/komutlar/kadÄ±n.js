...
exports.run = async (client, message, args) => {
 ...
 if (!message.member.roles.find(`id`, db.fetch(`kayıtçı_${message.guild.id}`))) return message.channel.sendEmbed(new Discord.RichEmbed().setTitle(isim).setDescription(`Bu Komutu Komutu Sadece ${kayıtçı} Rolündekiler Kullanabilir!`).setColor('2f3136').setFooter(`${client.user.username}`,`${client.user.avatarURL}`)  .setTimestamp())
  
  if (!member) return message.channel.sendEmbed(new Discord.RichEmbed().setTitle(isim).setDescription(`Birini Etiketlemelisin`).setColor('2f3136').setFooter(`${client.user.username}`,`${client.user.avatarURL}`)  .setTimestamp())
  if (!isim) return message.channel.sendEmbed(new Discord.RichEmbed().setTitle(isim).setDescription(`Bir isim yazmalısın!`).setColor('2f3136').setFooter(`${client.user.username}`,`${client.user.avatarURL}`)  .setTimestamp())
  member.setNickname(`${isim}`);
  let erkek = message.guild.roles.find(`id`, db.fetch(`bayanrol_${message.guild.id}`));  //kayıtsız_$
